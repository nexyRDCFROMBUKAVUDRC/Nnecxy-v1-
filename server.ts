/**
 * NNECXY - Production Full-Stack Server
 * Strictly adheres to PROMPT MAÎTRE FINAL NNECXY
 */
...
// Data directory & database setup
const DATA_DIR = path.join(process.cwd(), 'data');
const UPLOADS_DIR = path.join(process.cwd(), 'uploads');
const VIDEOS_DIR = path.join(UPLOADS_DIR, 'videos');
const AVATARS_DIR = path.join(UPLOADS_DIR, 'avatars');
const GROUPS_DIR = path.join(UPLOADS_DIR, 'group-images');
...
const DB_FILE = path.join(DATA_DIR, 'nnecxy_db.json');
...
interface DatabaseSchema {
  ...
  credentials: Record<string, { hash: string; salt: string }>; // userId -> auth
}
...
const uploadVideo = multer({
  ...
  fileFilter: (_req, file, cb) => {
    if (!file.mimetype.startsWith('video/')) {
      return cb(new Error('Veuillez sélectionner une vidéo.'));
    }
    cb(null, true);
  },
});
...
app.post('/api/auth/forgot-password', (req, res) => {
  ...
  // Instructions envoyées de manière sécurisée
  res.json({ success: true, message: 'Si cette adresse existe, un lien de réinitialisation vous a été envoyé.' });
});
...
app.get('/api/videos', (req, res) => {
  ...
  // Filtrer les vidéos visibles dans le feed public :
  // Non shadow-banned, visibilité 'public'
  let list = db.videos.filter((v) => !v.is_shadow_banned && v.visibility === 'public');

  // Tri par recommandation réelle : boost_score + likes*2 + comments*3 + freshness
  list.sort((a, b) => {
    const scoreA = (a.boost_score || 0) + (a.likes_count || 0) * 2 + (a.comments_count || 0) * 3 + new Date(a.created_at).getTime() / 10000000;
    const scoreB = (b.boost_score || 0) + (b.likes_count || 0) * 2 + (b.comments_count || 0) * 3 + new Date(b.created_at).getTime() / 10000000;
...
// GET /api/videos/my (Mes Vidéos - Section 16)
...
// GET /api/videos/user/:userId (Vidéos d'un créateur)
app.get('/api/videos/user/:userId', (req, res) => {
  const { userId } = req.params;
  const db = readDB();

  const userVideos = db.videos
    .filter((v) => v.user_id === userId && v.visibility !== 'private')
    .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());

  res.json({ videos: userVideos });
});

// POST /api/videos/upload (Section 14: Création d'une vidéo réelle)
app.post('/api/videos/upload', requireAuth, (req, res) => {
  uploadVideo.single('video')(req, res, (err) => {
    if (err) {
      return res.status(400).json({ error: err.message || 'Erreur lors de la sélection du fichier vidéo.' });
    }

    if (!req.file) {
      return res.status(400).json({ error: 'Veuillez sélectionner une vidéo.' });
    }
...
// DELETE /api/videos/:id (Section 17: Suppression réelle d'une vidéo)
app.delete('/api/videos/:id', requireAuth, (req, res) => {
  ...
  if (video.user_id !== currentUser.id) {
    return res.status(403).json({ error: 'Vous ne pouvez pas supprimer la vidéo d’un autre créateur.' });
  }
  ...
  writeDB(db);
  return res.json({ success: true, message: 'Vidéo supprimée avec succès.' });
});

// POST /api/videos/:id/like (Section 18: Likes réels)
app.post('/api/videos/:id/like', requireAuth, (req, res) => {
  ...
  if (existingLikeIndex !== -1) {
    ...
    // Notification si ce n'est pas sa propre vidéo
    if (video.user_id !== currentUser.id) {
...
// GET /api/videos/:id/comments (Section 19: Commentaires)
...
// POST /api/videos/:id/comments (Publier commentaire)
app.post('/api/videos/:id/comments', requireAuth, (req, res) => {
  ...
  if (video.user_id !== currentUser.id) {
    db.notifications.unshift({
      ...
      message: `@${currentUser.username} a commenté votre vidéo: "${content.slice(0, 30)}..."`,
...
// -----------------------------------------------------------------------------
// SIGNALEMENTS / MODERATION (Section 27 - PROMPT MAÎTRE EXACT)
// -----------------------------------------------------------------------------
...
app.post('/api/reports', requireAuth, (req, res) => {
  ...
  if (!video_id || !reason) {
    return res.status(400).json({ error: 'Veuillez préciser la vidéo et la raison du signalement.' });
  }
  ...
  // 1. Bloquer auto-signalement
  if (video.user_id === currentUser.id) {
    return res.status(400).json({ error: 'Vous ne pouvez pas signaler votre propre vidéo.' });
  }

  // 2. Bloquer double signalement
  ...
  // 3. LOGIQUE AUTOMATIQUE CRITIQUE (check_auto_moderation)
  // A) Compte les reports pour cette video_id avec la MÊME reason
  ...
  if (sameReasonCount >= 3) {
    ...
    return res.json({
      ...
      message: 'Merci, votre signalement a été reçu.',
    });
  }

  // B) Sinon, compte les reports avec des reasons DIFFÉRENTES
  const distinctReasons = new Set(db.reports.filter((r) => r.video_id === video_id).map((r) => r.reason));
  if (distinctReasons.size >= 5) {
    // DÉCLASSEMENT (is_shadow_banned = true, boost_score = 0, visibility = 'demoted')
...
// -----------------------------------------------------------------------------
// ABONNEMENTS / FOLLOWS (Section 21)
// -----------------------------------------------------------------------------
app.post('/api/users/:id/follow', requireAuth, (req, res) => {
  ...
  if (followIndex !== -1) {
    ...
    db.notifications.unshift({
      ...
      type: 'follow',
      title: 'Nouvel abonné',
      message: `@${currentUser.username} s'est abonné(e) à votre profil.`,
      is_read: false,
      created_at: new Date().toISOString(),
    });
  }

  // Recalcul exact des compteurs
...
// PROFILES & USERS
// -----------------------------------------------------------------------------
// GET /api/users/:id (Public Creator Profile - Section 11)
app.get('/api/users/:id', (req, res) => {
  ...
  // Public info ONLY (Section 11: Ne jamais afficher paramètres privés ou données financières)
  ...
  const videos = db.videos
    .filter(
      (v) =>
        !v.is_shadow_banned &&
        v.visibility === 'public' &&
        ((v.title && v.title.toLowerCase().includes(q)) || (v.description && v.description.toLowerCase().includes(q)))
    )
    .map((v) => {
      const creator = db.profiles.find((p) => p.id === v.user_id);
      return { ...v, creator };
...
// -----------------------------------------------------------------------------
// MESSAGING & CONVERSATIONS (Section 24)
// -----------------------------------------------------------------------------
  ...
  const list = db.conversations
    ...
    .map((c) => {
      const members = db.conversation_members
        .filter((cm) => cm.conversation_id === c.id)
        .map((cm) => db.profiles.find((p) => p.id === cm.user_id))
        .filter(Boolean);
...
// POST /api/conversations (Créer ou récupérer conversation)
app.post('/api/conversations', requireAuth, (req, res) => {
  ...
  db.conversation_members.push({ id: crypto.randomUUID(), conversation_id: newConvId, user_id: currentUser.id, role: 'member' });
  db.conversation_members.push({ id: crypto.randomUUID(), conversation_id: newConvId, user_id: target_user_id, role: 'member' });
...
app.post('/api/conversations/:id/messages', requireAuth, (req, res) => {
  ...
  if (!content || !content.trim()) {
    return res.status(400).json({ error: 'Le message ne peut être vide.' });
  }
...
// -----------------------------------------------------------------------------
// RECOMMANDATION & ÉVÉNEMENTS (Section 13)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// MONÉTISATION, BOOSTER, ADMOB (Sections 34, 35, 36, 37)
// STRICT REQUIREMENT: Ne jamais simuler de transactions, revenus ou faux succès.
// Afficher clairement : « Fonctionnalité momentanément indisponible. »
// -----------------------------------------------------------------------------
app.all(['/api/monetization', '/api/booster', '/api/creators/support', '/api/admob'], (_req, res) => {
  res.status(503).json({
    status: 'unavailable',
    error: 'Fonctionnalité momentanément indisponible.',
    message: 'Le module de paiement sécurisé sera activé prochainement. Aucune transaction fictive n’est simulée.',
  });
});
